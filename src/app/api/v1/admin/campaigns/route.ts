    ...
    handler: async () => ({
      ...
      body: {
        campaigns: campaignService.listAll().map((c) => ({
          ...
          max_tokens_per_customer: c.maxTokensPerCustomer,
...
const createSchema = z.object({
  ...
  max_tokens_per_customer: z.number().int().min(1).optional(),
  ...
  return runApiRoute({
    ...
    handler: async () => {
      ...
      const campaign = campaignService.create({
        ...
        maxTokensPerCustomer: body.max_tokens_per_customer ?? null,
